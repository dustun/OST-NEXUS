...
compose.yml     PostgreSQL и профиль backend CLI
...
Для создания первого администратора заполните **ADMIN_NAME**, **ADMIN_EMAIL** и **ADMIN_PASSWORD** в **backend/.env**, затем выполните **task admin:create**.
...
| **task shell** | открыть Bash внутри backend CLI-контейнера |
| **task admin:create** | создать или обновить администратора из backend/.env |
...
План вертикальных срезов описан в [дорожной карте](docs/roadmap.md), настройка доступа — в [руководстве администратора](docs/administration.md).
